...
class SaltyFishPPOConfig:
    ...
    log_interval: int = 1
    save_dir: str = "Y_Fu/checkpoints/saltyfish_baseline"
    logdir: str = "Y_Fu/logs/saltyfish_baseline"
    device: str = "auto"
...
class SaltyFishPPOTrainer:
    ...
    def collect_rollout(self, observation: np.ndarray) -> tuple[np.ndarray, dict[str, np.ndarray], dict[str, float]]:
        ...
        obs_buffer = np.zeros((steps, 1, self.env.obs_dim), dtype=np.float32)
        action_buffer = np.zeros((steps, 1), dtype=np.int64)
        logprob_buffer = np.zeros((steps, 1), dtype=np.float32)
        reward_buffer = np.zeros((steps, 1), dtype=np.float32)
        done_buffer = np.zeros(steps, dtype=np.float32)
        value_buffer = np.zeros((steps, 1), dtype=np.float32)
        ...
        for step in range(steps):
            ...
            next_observation, reward, done, info = self.env.step(action_int)

            action_buffer[step, 0] = action_int
            logprob_buffer[step, 0] = float(logprob.item())
            value_buffer[step, 0] = float(value.item())
            reward_buffer[step, 0] = float(np.mean(reward))
        ...
        metrics = {
            ...
            "team_possession_rate": event_sums["team_possession_steps"] / steps,
            "opponent_possession_rate": event_sums["opponent_possession_steps"] / steps,
            ...
            "attacking_third_possession_rate": event_sums["attacking_third_possession_steps"] / steps,
            "shot_actions": event_sums["shot_actions"],
            "shots_with_ball": event_sums["shots_with_ball"],
            "ball_out_losses": event_sums["ball_out_losses"],
            "mean_possession_reward_per_step": event_sums["possession_reward"] / steps,
            "mean_pass_reward_per_step": event_sums["pass_reward"] / steps,
            "mean_carry_reward_per_step": event_sums["carry_reward"] / steps,
            "mean_territory_reward_per_step": event_sums["territory_reward"] / steps,
            "mean_shot_reward_per_step": event_sums["shot_reward"] / steps,
            "mean_out_penalty_per_step": event_sums["out_penalty"] / steps,
            "mean_shaping_reward_per_step": event_sums["shaping_reward"] / steps,
        }
        ...
        for _ in range(self.config.update_epochs):
            ...
            for start in range(0, batch_size, minibatch_size):
                ...
                value_clipped = old_values[mb] + torch.clamp(
                    value_delta, -self.config.clip_coef, self.config.clip_coef
                )
    ...
    def train(self) -> Path:
        steps_per_update = self.config.rollout_steps
        ...
        for update in range(1, num_updates + 1):
            ...
            if update % self.config.log_interval == 0:
                ...
                print(
                    ...
                    f"poss_gain={rollout_metrics['possession_gains']:.0f} "
                    f"poss_loss={rollout_metrics['possession_losses']:.0f} "
                    f"team_poss={rollout_metrics['team_possession_rate']:.3f} "
                    f"opp_poss={rollout_metrics['opponent_possession_rate']:.3f} "
                    ...
                    f"ball_out={rollout_metrics['ball_out_losses']:.0f} "
                    f"poss_r={rollout_metrics['mean_possession_reward_per_step']:.4f} "
                    f"pass_r={rollout_metrics['mean_pass_reward_per_step']:.4f} "
                    f"carry_r={rollout_metrics['mean_carry_reward_per_step']:.4f} "
                    f"third_r={rollout_metrics['mean_territory_reward_per_step']:.4f} "
                    f"shot_r={rollout_metrics['mean_shot_reward_per_step']:.4f} "
                    f"out_p={rollout_metrics['mean_out_penalty_per_step']:.4f} "
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train SaltyFish-inspired single-player baseline.")
    ...
    parser.add_argument("--save-dir", default="Y_Fu/checkpoints/saltyfish_baseline")
    parser.add_argument("--logdir", default="Y_Fu/logs/saltyfish_baseline")
