...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate SaltyFish-style 5v5 baseline.")
...
def make_env(args: argparse.Namespace, config: dict) -> ReducedActionFootballEnv:
    ...
    env_config = CompetitionEnvConfig(
        env_name=config.get("env_name", "5_vs_5"),
        representation=config.get("representation", "simple115v2"),
        rewards=config.get("rewards", "scoring"),
        num_controlled_players=int(config.get("num_controlled_players", 4)),
        use_engineered_features=bool(config.get("use_engineered_features", True)),
        reward_config=CompetitionRewardConfig(
            ...
            carry_progress_reward_scale=float(config.get("carry_progress_reward_scale", 0.04)),
            ...
            attacking_risk_x_threshold=float(config.get("attacking_risk_x_threshold", 0.4)),
            attacking_loss_penalty_scale=float(config.get("attacking_loss_penalty_scale", 0.5)),
            out_of_play_loss_penalty=float(config.get("out_of_play_loss_penalty", 0.05)),
        ),
    )
...
def evaluate(
    env: ReducedActionFootballEnv,
    model: StructuredSimple115ActorCritic | None,
    device: torch.device,
    episodes: int,
    deterministic: bool,
) -> dict[str, float]:
