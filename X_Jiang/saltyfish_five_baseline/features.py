...
def _compute_offside_flags(raw_obs: dict[str, Any]) -> np.ndarray:
    left_team_raw = np.asarray(raw_obs["left_team"], dtype=np.float32)
    right_team_raw = np.asarray(raw_obs["right_team"], dtype=np.float32)
...
def build_engineered_features(raw_obs: dict[str, Any]) -> np.ndarray:
    left_team_raw = np.asarray(raw_obs["left_team"], dtype=np.float32)
    left_dir_raw = np.asarray(raw_obs["left_team_direction"], dtype=np.float32)
    right_team_raw = np.asarray(raw_obs["right_team"], dtype=np.float32)
    left_team = _pad_2d(left_team_raw, MAX_PLAYERS_PER_TEAM)
